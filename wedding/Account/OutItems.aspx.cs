...
public partial class Account_Vendors : System.Web.UI.Page
{
    ...
    {
        DataTable Grid_dt = new DB().Admin_OutCategorys_GetByCat_ID(int.Parse(Drop_Sub.SelectedValue));
    ...
    private void FillGrid()
    {
        try
        {
            ...
            if (Grid_dt.Rows.Count == 0)
            {
                DataTable Grid_dt2 = new DB().Admin_OutItems_GetByMenu_ID(int.Parse(Drop_Sub.SelectedValue));
                grid_Display.DataSource = Grid_dt2;
                grid_Display.DataBind();
            }
            else
            {
                DataTable Grid_dt2 = new DB().Admin_OutItems_GetByCat_ID(int.Parse(DropDownList1.SelectedValue));
            grid_Display.DataSource = Grid_dt2;
            grid_Display.DataBind();
            }
        ...
        try
        {
            ...
            if (lbl_id.Text == "")
            {
                if (upload_image.HasFile)
                {
                    ...
                    thumb_url = (@"/images/" + upload_image.FileName);
                        ...
                        if (Grid_dt.Rows.Count == 0)
                        {
                            new DB().Admin_OutItems_Insert(txt_Title.Text, thumb_url, 0, int.Parse(Drop_Sub.SelectedValue));
                        }
                        else
                        {
                            new DB().Admin_OutItems_Insert(txt_Title.Text, thumb_url, int.Parse(DropDownList1.SelectedValue), 0);
                        }
            ...
            else
            {
                ...
                if (valid)
                {
                    if (visibleDrop.Visible == false)
                    {
                        new DB().Admin_OutItems_Update(int.Parse(lbl_id.Text), txt_Title.Text, thumb_url, 0, int.Parse(Drop_Sub.SelectedValue));
                    }
                    else
                    {
                        new DB().Admin_OutItems_Update(int.Parse(lbl_id.Text), txt_Title.Text, thumb_url, int.Parse(DropDownList1.SelectedValue), 0);

                    }
                    DataTable edit_dt = new DB().ADMIN_OutItems_GetByID(int.Parse(lbl_id.Text));
                    Bind(edit_dt);
                    FillGrid();
                    lbl_id.Text = "";
                    image_thumb.Visible = false;
    ...
    protected void grid_Display_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            new DB().ADMIN_OutItems_Delete(id);
            FillGrid();
        }
        catch (Exception ex)
        {
    ...
    protected void grid_Display_RowEditing(object sender, GridViewEditEventArgs e)
    {
        ...
        DataTable edit_dt = new DB().ADMIN_OutItems_GetByID(id);
        Bind(edit_dt);
