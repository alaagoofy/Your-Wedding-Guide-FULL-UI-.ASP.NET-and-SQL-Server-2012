...
public partial class Account_Default : System.Web.UI.Page
{
    ...
    protected void btn_save_Click(object sender, EventArgs e)
    {
        try
        {
            if (lbl_id.Text == "")
            {
                ...
                if (valid)
                {
                    new DB().Admin_OutCategory_Insert(txt_Title.Text, int.Parse(Drop_Sub.SelectedValue));
                    FillGrid();
                    clear();
                    lbl_id.Text = "";
            ...
            {
                new DB().Admin_OutCategory_Update(int.Parse(lbl_id.Text), txt_Title.Text, int.Parse(Drop_Sub.SelectedValue));
                DataTable edit_dt = new DB().ADMIN_OutCategory_GetByID(int.Parse(lbl_id.Text));
                Bind(edit_dt);
                FillGrid();
                lbl_id.Text = "";
                lbl_error.Text = "saved successfully";
    ...
    {
        DataTable Grid_dt = new DB().ADMIN_OutCategory_GetByLaung(1);
    ...
    protected void grid_Display_RowEditing(object sender, GridViewEditEventArgs e)
    {
        ...
        DataTable edit_dt = new DB().ADMIN_OutCategory_GetByID(id);
        Bind(edit_dt);
    ...
    protected void grid_Display_RowDeleting1(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            new DB().ADMIN_OutCategory_Delete(id);
            FillGrid();
        }
        catch (Exception ex)
        {
